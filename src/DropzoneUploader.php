...
class DropzoneUploader extends \Nette\Application\UI\Control
{
    ...
    public function process(\Nette\Application\UI\Form $form, $values)
    {
        ...
        if ($file->isImage()) {
            $image = $file->toImage();
            $width = $this->photo['width'] ?? null;
            $height = $this->photo['height'] ?? null;
            $flags = $this->photo['flags'] ?? null;

            if (!is_null($width) || !is_null($height)) {
                $image->resize($width, $height, $flags);
            }
            $image->save($targetPath . DIRECTORY_SEPARATOR . $name, $this->photo['quality'] ?? null, $this->photo['type'] ?? null);
        } else {
